...
switch($_POST['request']){
            ...
            while($data = mysqli_fetch_array($result)){
                ...
                foreach($list_vehicule as $vehicule){
                    ...
                    $html .= '<tr id="numInt-'.$interv.'"> 
                            ...
                            <th class="border border-y-slate-700"><a class="underline text-blue-600" href="liste_interventions.php?token='.$interv.'">
                            Intervention terminée</a></th></tr>';
                    $status = 1;
                    }
                }
                }             
        ...
        function URLCouranteSansParametres(){
            $urlCourante=$_SERVER["REQUEST_URL"];
        ...
        case 'interventionFinie':
            ...
            $requete = "UPDATE `vehicules` SET `fin_inter`='". mysqli_real_escape_string($GLOBALS['Database'],$status) ."' WHERE `id_inter`='".mysqli_real_escape_string($GLOBALS['Database'],$idInter)."'";
            $result = mysqli_query($GLOBALS['Database'], $requete)or die;
            error_log($requete);
            $msg = 'véhicule terminé';
            $url = URLCouranteSansParametres();
