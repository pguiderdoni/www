            ...
            while($data = mysqli_fetch_array($result)){
                ...
                foreach($list_vehicule as $vehicule){
                    ...
                    $html .= '<tr> 
                                ...
                                <th class="border border-y-slate-700 py-2 text-xs"><a type="btn" onclick="interventionOk('.$interv.');" class="underline text-blue-600" href="liste_interventions.php">Intervention terminée</a></th>
                            </tr>';
                    $status = 1;
                    }
                }
        ...
        case 'finishLoad':
            ...
            while($data = mysqli_fetch_array($result)){
                ...
                foreach($list_vehicule as $vehicule){
                    ...
                    $modele = $data2['nom_modele'];
                    $html .= '<tr>
                                <th class="border border-y-slate-700 py-2 text-xs"><a onclick="facturation('.$interv.');" class="underline text-blue-600" href="base.php">Payer</a></th>
                                <th class="border border-y-slate-700 py-2 text-xs">'.$interv.'</th>
                                <th class="border border-y-slate-700 py-2 text-xs">'.$marque.'</th>
                                <th class="border border-y-slate-700 py-2 text-xs">'.$modele.'</th>
                                <th class="border border-y-slate-700 py-2 text-xs">'.$immat.'</th>
                                <th class="border border-y-slate-700 py-2 text-xs"><a onclick="interventionNonOk('.$interv.');" class="underline text-blue-600" href="base.php">Non terminée</a></th>
                            </tr>';
                    $status = 1;
                    }
                }
